...
pub const Renderer = struct {
    ...
    pub fn render(self: *Renderer, current: *const Screen) !void {
        ...
        var buf_writer: std.Io.Writer.Allocating = .fromArrayList(self.allocator, &self.output_buffer);
        defer self.output_buffer = buf_writer.toArrayList();

        // Hide cursor during rendering
        try buf_writer.writer.writeAll(terminal.Escape.hide_cursor);

        if (self.prev_buffer) |*prev| {
            // Diff-based rendering
            if (prev.width == current.width and prev.height == current.height) {
                try self.renderDiff(current, prev, &buf_writer.writer);
            } else {
                // Screen size changed, full redraw
                try prev.resize(current.width, current.height);
                try self.renderFull(current, &buf_writer.writer);
            }
        } else {
        ...
        // Reset style at end
        try buf_writer.writer.writeAll(terminal.Escape.reset);

        // Show cursor if needed
        try buf_writer.writer.writeAll(terminal.Escape.show_cursor);
